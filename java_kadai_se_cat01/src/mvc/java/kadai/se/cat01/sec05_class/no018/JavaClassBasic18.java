...
/*
 * ◆別クラス(オブジェクト)の利用
 * JavaClassBasic18Objクラスは次のフィールドを保有しています。
 * ・fName      (苗字/String型)
 * ・lName      (名前/String型)
 * ・age        (年齢/int型)
 * ・gender     (性別/char型)
 ...
 * ・tel        (電話番号/String型)
 *
 * JavaClassBasic18Objクラスの各フィールドにデータを格納し、
 ...
 * ===============================
 *
 * ※showUserInfoメソッド、JavaClassBasic18Obj.javaの編集不可
 *
 */
public class JavaClassBasic18{
	...
	public static void main(String[] args) {
		...
		JavaClassBasic18Obj sub = new JavaClassBasic18Obj();
		sub.setAddress(address);
		sub.setAge(age);
		sub.setfName(fName);
		sub.setlName(lName);
		sub.setGender(gender);
		sub.setPostalCode(postalCode);
		sub.setTel(tel);
	...
	public static void showUserInfo(JavaClassBasic18Obj uco) {

		System.out.println("◆JavaClassBasic18");
